...
export const customerAddsServiceMatchScoreCalculation = async (serviceId: string, serviceType: string) => {
  if (serviceType == "Lawn") {
    ...
    businessServices.forEach(async (businessService) => {
      ...
      let totalCoverageScore =
        (-25 / service.totalCoverage ** 2) * (businessService.totalCoverage - 2 * service.totalCoverage) ** 2 + 100;
    ...
    businessServices.forEach(async (businessService) => {
      ...
      let GBMatchScore =
        (-25 / service.GBPerMonth ** 2) * (businessService.GBPerMonth - 2 * service.GBPerMonth) ** 2 + 100;
      if (GBMatchScore > 100) {
        GBMatchScore = 100;
...
export const businessAddsServiceMatchScoreCalculation = async (businessId: string, businessType: string) => {
  ...
  if (businessType == "Lawn") {
    ...
    customerServices.forEach(async (customerService) => {
      ...
      let totalCoverageScore =
        (-25 / customerService.totalCoverage ** 2) * (business.totalCoverage - 2 * customerService.totalCoverage) ** 2 +
        100;
      if (totalCoverageScore > 100) {
        totalCoverageScore = 100;
      } else if (totalCoverageScore < 0) {
    ...
    customerServices.forEach(async (customerService) => {
      ...
      let costMatchScore = -(25 / customerService.costPerMonth ** 2) * business.costPerMonth ** 2 + 100;
      let speedMatchScore =
        (-25 / customerService.speed ** 2) * (business.speed - 2 * customerService.speed) ** 2 + 100;
      if (speedMatchScore > 100) {
        speedMatchScore = 100;
      } else if (speedMatchScore < 0) {
        let speedMatchScore = 0;
    ...
    customerServices.forEach(async (customerService) => {
      ...
      let GBMatchScore =
        (-25 / customerService.GBPerMonth ** 2) * (business.GBPerMonth - 2 * customerService.GBPerMonth) ** 2 + 100;
      if (GBMatchScore > 100) {
        GBMatchScore = 100;
      } else if (GBMatchScore < 0) {
        GBMatchScore = 0;
...
export const editMatchScoreBusiness = async (businessId: string, businessType: string) => {
  if (businessType == "Lawn") {
    const business = await prisma.businessInternet.findUnique({
      where: { id: businessId },
    });
...
export const deleteMatchScoreBusiness = async (serviceId: string) => {
