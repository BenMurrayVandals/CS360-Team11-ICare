  ...
  try {
    ...
    switch (serviceType) {
      ...
      default: //Interior case
        try {
          const { costPerMonth, sqFootage } = await readBody(event);
          //Make sure we have all important data
          if (!costPerMonth || !sqFootage) {
            throw createError({ statusCode: 400, message: "Bad Request: Missing required fields" });
          }
          const updatedInteriorService = await prisma.customerInterior.update({
          ...
          await editMatchScoreCustomer(updatedInteriorService.id, updatedInteriorService.serviceType);
