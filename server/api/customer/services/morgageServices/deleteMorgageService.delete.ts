...
export default defineEventHandler(async (event) => {
  try {
    ...
    const existingMorgageService = await prisma.customerMorgage.findUnique({
      where: {
        id: id,
      },
    });
    ...
    // Check if the customerId matches the customerId associated with the morgage service
    if (existingMorgageService.customerId !== customerId) {
      throw createError({ statusCode: 403, message: "Forbidden: Incorrect customerId" });
    }

    // Delete the morgage service based on ID and customer ID
    const deletedMorgageService = await prisma.customerMorgage.delete({
      where: {
        id: id,
      },
    });
    await deleteMatchScoreCustomer(deletedMorgageService.id);
