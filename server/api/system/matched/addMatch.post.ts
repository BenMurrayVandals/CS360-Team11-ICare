...
export default defineEventHandler(async (event) => {
  try {
    ...
    const { businessId, customerId, serviceType, cserivceId, bserviceId, matchScore } = await readBody(event);
    //Make sure we have all important data
    if (!businessId || !customerId || !serviceType || !cserivceId || !bserviceId || !matchScore) {
